...
- Never convert all `DbUpdateException` to a domain exception. Inside the catch, re-verify the specific condition (e.g. `AnyAsync`) and only throw the domain exception if confirmed; otherwise `throw;` to re-surface the original error.
